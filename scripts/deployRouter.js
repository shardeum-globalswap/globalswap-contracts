...
async function createPair(factory, tokenA, tokenB) {
  ...
  const contract = new ethers.Contract(factory, FactoryABI, deployer);
  const createPairTx = await contract.createPair(tokenA, tokenB);
...
async function addLiquidity(
  ...
  const deadline = ethers.BigNumber.from(time); // convert s to ms
